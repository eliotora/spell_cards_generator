...
def get_chromium_path():
    if getattr(sys, "frozen", False):
        ...
        chrome_dirs = list(
            (base_path / "ms-playwright").glob("chromium-*/chrome-win/chrome.exe")
        )
        if chrome_dirs:
...
def html_to_pdf(html_path, output_filename):
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(executable_path=str(chromium_exe), headless=True)
...
def exporter_pdf(spells, path, mode="rules", show_source=False, show_VO_name=False):
    ...
    html_export(
        spells, html_path, mode, show_source=show_source, show_VO_name=show_VO_name
    )
