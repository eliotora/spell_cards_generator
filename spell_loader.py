                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as file:
                            ...
                            for spell_name in class_data.get("sorts", []):
                                class_lookup.setdefault(spell_name, set()).add(class_data.get("classe", ""))
            ...
            if filename.endswith('.json'):
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as file:
                        ...
                        spell = {field: spell_data.get(field, "") for field in SPELL_FIELDS}
                        spell["source"] = source_folder
                        spell["classes"] = sorted(class_lookup.get(spell_data.get("nom", ""), []))
                        spells.append(spell)
                except (json.JSONDecodeError, IOError) as e:
