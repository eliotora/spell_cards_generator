...
class SpellModel(BaseModel, ExplorableMixin, JsonMixin, PopupMixin, ExportableMixin):
    ...
    def to_html_dict(self):
        ...
        result["italics"] = [
            f"niveau {self.level} - {self.schools[0] if len(self.schools) < 2 else f"{self.schools[0]} ({", ".join(self.schools[1:])})" }{f" (rituel)" if self.ritual else ""}"
        ]
        result["bolds"] = [
            f"<strong>{self.__class__.__dataclass_fields__[field].metadata.get(ExplorableMixin.METADATA_NAMESPACE).label}</strong> : {self.__getattribute__(field) if not isinstance(self.__getattribute__(field), list) else ", ".join(self.__getattribute__(field))}"
            for field in ["casting_time", "range", "components", "duration"]
        ]
        result["main_text"] = (
            f"{self.description}{f"<br><strong>Aux niveaux supérieurs. </strong>{self.at_higher_levels}" if self.at_higher_levels else ""}"
        )
        result["footer"] = [
            f"<div class='classe'>{classe}</div>" for classe in self.classes
        ]
...
MODEL_EXPORT_MODE_HTML_FILES[
    (SpellModel.__name__.lower(), ExportOption.GRIMOIRE.value)
] = "spell_grimoire.html"
